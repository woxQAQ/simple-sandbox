...
class TestPerformance:
    ...
    def execute_code_with_timing(
        self, client, language: str, code: str, **kwargs
    ) -> Dict[str, Any]:
        """执行代码并记录时间"""
        ...
        if "environment_variables" in kwargs:
            request_data["environment_variables"] = kwargs[
        ...
        for test_case in test_cases:
            ...
            for _ in range(5):
                result = self.execute_code_with_timing(
                    client, test_case["language"], test_case["code"]
                )
    ...
    def test_concurrent_execution_performance(self, client):
        ...
        for concurrency in concurrency_levels:
            ...
            if successful_requests:
                ...
                print(
                    f"  Throughput: {len(successful_requests)/total_time:.2f} req/s"
                )
    ...
    def test_memory_usage_performance(self, client):
        ...
        for test_case in memory_test_cases:
            ...
            if result["response"]["status"] == "success":
                ...
                # 内存使用应该在合理范围内
                assert (
                    memory_used_mb <= test_case["expected_memory_mb"] * 2
                )  # 允许2倍误差
...
""",
        ...
        for test_case in cpu_test_cases:
            ...
            if result["response"]["status"] == "success":
                # CPU密集型任务应该在合理时间内完成
                assert (
                    result["execution_time"] <= test_case["max_execution_time"]
                )
                assert (
                    result["api_response_time"]
                    <= test_case["max_execution_time"] + 5
                )
            elif result["response"]["status"] == "timeout":
                # 如果超时，应该接近超时时间
                assert (
                    result["execution_time"]
                    >= test_case["max_execution_time"] * 0.9
                )
...
"""
        ...
        # 分析结果
        successful_results = [
            r for r in results if r["response"]["status"] == "success"
        ]
        success_rate = len(successful_results) / len(results)

        if successful_results:
            avg_response_time = statistics.mean(
                [r["api_response_time"] for r in successful_results]
            )
            avg_execution_time = statistics.mean(
...
"""
        ...
        if len(memory_usages) >= 5:
            ...
            print(
                f"  Memory variance: {memory_variance/((1024*1024)**2):.2f} MB²"
            )

            # 内存使用应该相对稳定
            coefficient_of_variation = (
                memory_std / avg_memory if avg_memory > 0 else 0
            )
    ...
    def test_response_time_consistency(self, client):
        ...
        if len(response_times) >= 10:
            ...
            # 响应时间应该相对一致
            coefficient_of_variation = (
                std_time / avg_time if avg_time > 0 else 0
            )
            assert coefficient_of_variation < 1.0  # 变异系数小于100%
            assert (
                max_time - min_time < avg_time * 2
            )  # 最大差异不超过平均时间的2倍
