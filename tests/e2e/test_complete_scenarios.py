        ...
        class APIClient:
            ...
            def execute_code(self, language: str, code: str, **kwargs):
                ...
                if "environment_variables" in kwargs:
                    data["environment_variables"] = kwargs[
                        "environment_variables"
                    ]

                response = self.session.post(
                    f"{self.base_url}/api/v1/execute", json=data
                )
                return response
...
"""
        ...
        response = api_client.execute_code(
            "python", code, input_data=input_data
        )
...
"""
        ...
        env_vars = {"API_KEY": "test_api_key_123", "DEBUG": "true"}

        response = api_client.execute_code(
            "python", code, environment_variables=env_vars
        )
...
"""
    ...
    def test_multiple_requests_scenario(self, api_client):
        ...
        for i, test_case in enumerate(test_cases):
            response = api_client.execute_code(
                test_case["language"], test_case["code"]
            )
...
"""
        ...
        response = api_client.execute_code(
            "python", code, timeout=120, memory_limit=256  # 2分钟超时
        )
...
"""
        ...
        response = api_client.execute_code(
            "python", code, timeout=60, memory_limit=512  # 较大的内存限制
        )
    ...
    def test_error_recovery_scenario(self, api_client):
        ...
        # 第一步：执行有错误的代码
        error_code = (
            "print('Before error'); invalid_syntax_here; print('After error')"
        )
        ...
        # 第二步：执行正确的代码
        success_code = (
            "print('Recovery successful'); print('System is working')"
        )
