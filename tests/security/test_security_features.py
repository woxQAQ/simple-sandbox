    ...
    def test_dangerous_imports_blocked(self, client):
        ...
        for code in dangerous_codes:
            ...
            if data["status"] == "error":
                ...
                # 检查是否包含安全相关的错误信息
                security_keywords = ["import", "module", "permission", "denied", "blocked"]
...
"""
        ...
        if data["status"] == "error":
            ...
            # 如果成功，内存使用应该在限制范围内
            assert data["memory_used"] <= 64 * 1024 * 1024 * 1.1  # 允许10%的误差
...
"""
    ...
    def test_file_system_isolation(self, client):
        ...
        for code in file_access_codes:
            ...
            if data["status"] == "error":
                error_msg = (data["stderr"] + " " + (data["error"] or "")).lower()
                permission_keywords = ["permission", "denied", "no such file", "not found"]
                assert any(keyword in error_msg for keyword in permission_keywords)
