...
def generate_syscall_array(language: str, syscall_names: List[str]) -> str:
    ...
    if unknown_syscalls:
        print(f"Warning: Unknown syscalls for {language}: {unknown_syscalls}", file=sys.stderr)
...
def main():
    ...
    for filename in os.listdir(syscalls_dir):
        if filename.endswith('.json'):
            ...
            config_files.append((language, os.path.join(syscalls_dir, filename)))
    ...
    print("#endif")
    print()
    print("#define SYSCALL_COUNT (sizeof(ALLOWED_SYSCALLS) / sizeof(ALLOWED_SYSCALLS[0]))")
