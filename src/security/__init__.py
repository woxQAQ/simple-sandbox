...
class SecurityManager:
    ...
    def setup_security_profile(self, language: str, uid: int, gid: int):
        ...
        if not syscalls:
            raise SecurityError(f"No syscall configuration found for language: {language}")
        
        # 验证系统调用列表
        if not self.validate_syscalls(syscalls):
            raise SecurityError(f"Invalid syscall configuration for language: {language}")
...
def create_secure_process(language: str, uid: int, gid: int, 
                         config_dir: str = None, library_path: str = None):
    """
    创建安全进程的便利函数
