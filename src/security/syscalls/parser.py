...
class SyscallConfigParser:
    ...
    def __init__(self, config_dir: str = None):
        if config_dir is None:
            # 默认使用build/seccomp目录
            self.config_dir = Path(__file__).parent.parent.parent.parent / "build" / "seccomp"
        else:
    ...
    def _load_all_configs(self):
        ...
        if not self.config_dir.exists():
            logger.warning(f"Syscall config directory not found: {self.config_dir}")
            return
    ...
    def _parse_config_file(self, config_file: Path) -> Set[str]:
        ...
        if 'syscalls' in config:
            for syscall_group in config['syscalls']:
                if 'names' in syscall_group and syscall_group.get('action') == 'SCMP_ACT_ALLOW':
    ...
    def _get_basic_syscalls(self) -> List[str]:
        ...
        return [
            ...
            "rt_sigprocmask", "getpid", "getuid", "getgid", "geteuid", "getegid",
            "exit", "exit_group"
        ]
