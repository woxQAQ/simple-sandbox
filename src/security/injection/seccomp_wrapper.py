...
class SeccompInjectionError(Exception):
    ...
    def __init__(self, error_code: int, message: str = None):
        ...
        super().__init__(f"Seccomp injection failed: {self.message} (code: {error_code})")
...
class SeccompInjector:
    ...
    def _setup_function_signatures(self):
        ...
        self._lib.apply_seccomp_filter.argtypes = [ctypes.POINTER(ctypes.c_int), ctypes.c_size_t]
        ...
        self._lib.inject_seccomp_profile.argtypes = [
            ctypes.POINTER(ctypes.c_int), ctypes.c_size_t, ctypes.c_uint32, ctypes.c_uint32
        ]
    ...
    def drop_privileges(self, uid: int, gid: int) -> None:
        ...
        result = self._lib.drop_privileges(ctypes.c_uint32(uid), ctypes.c_uint32(gid))
        if result != SECCOMP_SUCCESS:
            raise SeccompInjectionError(result)
    ...
    def apply_seccomp_filter(self, syscalls: List[int]) -> None:
        ...
        if not syscalls:
            raise SeccompInjectionError(SECCOMP_ERROR_INVALID_ARGS, "Syscall list cannot be empty")
    ...
    def inject_seccomp_profile(self, syscalls: List[int], uid: int, gid: int) -> None:
        """完整的seccomp注入流程"""
        if not self._lib:
            raise SeccompInjectionError(SECCOMP_ERROR_UNSUPPORTED)
        ...
        result = self._lib.inject_seccomp_profile(
            syscall_array, len(syscalls), ctypes.c_uint32(uid), ctypes.c_uint32(gid)
        )
        if result != SECCOMP_SUCCESS:
            raise SeccompInjectionError(result)
...
# 便利函数
def inject_seccomp_for_language(language: str, uid: int, gid: int, 
